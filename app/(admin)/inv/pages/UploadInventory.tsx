...
import type { UploadError, InventoryItem } from '@/app/(admin)/inv/types/inventory';
...
import { uploadExcelFile, retryFailedRows, setUploadErrors, setLastUploadSummary, addInventoryItem } from '@/redux/slices/inv.slice';
...
const UploadInventory: React.FC = () => {
  ...
  const handleImportAll = () => {
    ...
    validRows.forEach(row => dispatch(addInventoryItem(row)));
    setStep('done');
  ...
  const handleImportRetryRows = () => {
    if (!uploadErrors) return;
    const validRows = uploadErrors
      .filter(e => Object.keys(e.errors).length === 0 && !skippedRows.has(uploadErrors.indexOf(e)))
      .map(e => ({
        ...e.data,
  ...
  const { uploadErrors, lastUploadSummary, loading } = useAppSelector(state => state.inventory);
  ...
  const handleFileSelected = async (fileAsset: any) => {
    ...
    normalizedHeaders.forEach((uploaded, idx) => {
  ...
  const handleProceedToPreview = () => {
    ...
    const errors = mapped.map((row, idx) => {
  ...
  const handleUpdateFailedRow = (idx: number, updated: Record<string, any>) => {
    if (!uploadErrors) return;
    const newErrors = uploadErrors.map((err, i) =>
