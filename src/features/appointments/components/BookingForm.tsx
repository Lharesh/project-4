        ...
        return (
            ...
            >
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16, background: '#f5f5f5', borderRadius: 8, padding: 12 }}>
                    <span className="material-icons" style={{ fontSize: 28, marginRight: 8, color: COLORS.vata[700] }}>person</span>
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <div style={{ fontWeight: 600, textAlign: 'left' }}>{values.clientName}</div>
                        <div style={{ color: '#666', fontSize: 14, textAlign: 'left' }}><span className="material-icons" style={{ fontSize: 18, verticalAlign: 'middle', color: COLORS.vata[700] }}>phone</span> {values.clientMobile}</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', marginLeft: 12 }}>
                        <span style={{ fontWeight: 500, background: '#e0e0e0', borderRadius: '50%', padding: '10px 16px', fontSize: 16, color: COLORS.vata[700], display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{values.clientId}</span>
                    </div>
                </div>
                ...
                <div style={{ position: 'relative', marginBottom: 12 }}>
                    <input
                        ...
                        value={therapySearch}
                        onChange={e => { setTherapySearch(e.target.value); handleChange('selectedTherapy', ''); }}
                ...
                <div style={{ marginBottom: 12, overflowX: 'auto', whiteSpace: 'nowrap' }}>
                    {filteredTherapists.map((t: any) => (
                        <button type="button" key={t.id} style={{ background: values.selectedTherapists.includes(t.id) ? '#1976d2' : '#eee', color: values.selectedTherapists.includes(t.id) ? '#fff' : '#222', borderRadius: 4, padding: '6px 14px', border: 'none', marginRight: 8, cursor: 'pointer' }} onClick={() => handleChange('selectedTherapists', [t.id])}>{t.name}</button>
                    ))}
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                    <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
                        <span className="material-icons" style={{ position: 'absolute', left: 8, color: COLORS.vata[700], fontSize: 20 }}>event</span>
                        <input
                            type="date"
                            value={values.startDate}
                            onChange={e => handleChange('startDate', e.target.value)}
                            onBlur={() => handleBlur('startDate')}
                            style={{ width: '100%', padding: '8px 8px 8px 32px', borderRadius: 6, border: '1px solid #ccc', outline: errors.startDate && touched.startDate ? '2px solid red' : undefined }}
                        />
                        {touched.startDate && errors.startDate && <div style={{ color: 'red', fontSize: 12, position: 'absolute', top: 40 }}>{errors.startDate}</div>}
                    </div>
                    <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
                        <span className="material-icons" style={{ position: 'absolute', left: 8, color: COLORS.vata[700], fontSize: 20 }}>schedule</span>
                        <input
                            type="time"
                            value={values.timeSlot}
                            onChange={e => handleChange('timeSlot', e.target.value)}
                            onBlur={() => handleBlur('timeSlot')}
                            style={{ width: '100%', padding: '8px 8px 8px 32px', borderRadius: 6, border: '1px solid #ccc', outline: errors.timeSlot && touched.timeSlot ? '2px solid red' : undefined }}
                        />
        ...
        <ScrollView contentContainerStyle={{ padding: 16 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16, backgroundColor: '#f5f5f5', borderRadius: 8, padding: 12 }}>
                ...
                <View style={{ flex: 1, flexDirection: 'column', alignItems: 'flex-start' }}>
                    <Text style={{ fontWeight: '600', textAlign: 'left' }}>{values.clientName}</Text>
                    <Text style={{ color: '#666', fontSize: 14, textAlign: 'left' }}><MaterialIcon name="phone" size={18} color={COLORS.vata[700]} /> {values.clientMobile}</Text>
                </View>
                <View style={{ marginLeft: 12, backgroundColor: '#e0e0e0', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 10, alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ fontWeight: '500', fontSize: 16, color: COLORS.vata[700] }}>{values.clientId}</Text>
                </View>
            </View>
            {/* Therapy Smart Search */}
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
                <View style={{ flex: 1, position: 'relative', flexDirection: 'row', alignItems: 'center' }}>
                    <MaterialIcon name="event" size={20} style={{ position: 'absolute', left: 8, color: COLORS.vata[700] }} />
                    {DateTimePicker ? (
                ...
                <View style={{ flex: 1, position: 'relative', flexDirection: 'row', alignItems: 'center' }}>
                    <MaterialIcon name="schedule" size={20} style={{ position: 'absolute', left: 8, color: COLORS.vata[700] }} />
                    {DateTimePicker ? (
