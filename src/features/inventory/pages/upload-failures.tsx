...
import { retryFailedRows } from '../inventorySlice';
import type { RootState, AppDispatch } from '../../../redux/store';
import type { UploadError } from '../types/inventory';

const UploadFailuresPage = () => {
  const dispatch = useDispatch<AppDispatch>();
  const uploadErrors = useSelector((state: RootState) => state.inventory.uploadErrors as UploadError[]);
  ...
  return (
    <View style={styles.container}>
      ...
      {uploadErrors.length === 0 ? (
        ...
        uploadErrors.map((err: UploadError) => (
